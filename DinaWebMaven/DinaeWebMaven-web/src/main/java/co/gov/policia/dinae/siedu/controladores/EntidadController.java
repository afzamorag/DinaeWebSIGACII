...
public class EntidadController extends AbstractController {
    ...
    public String initReturnCU() {
        ...
        releaseControllers();
        return navigationFaces.redirectSIEDUEntidad();
    }
    ...
    public void onSave() {
        ...
        try {            
            if (getSelected().getEntiEnti() == null) {
                this.selected.setEntiUsuCrea(this.loginFaces.getPerfilUsuarioDTO().getUsuarioLogueado());
                service.create(getSelected());
            } else {
                this.selected.setEntiUsuMod(this.loginFaces.getPerfilUsuarioDTO().getUsuarioLogueado());
