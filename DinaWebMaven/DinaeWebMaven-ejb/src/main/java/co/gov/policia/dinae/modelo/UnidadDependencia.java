...
@NamedQueries({
  ...
  @NamedQuery(name = "UnidadDependencia.findEscuelaVigente", query = "SELECT u FROM UnidadDependencia u WHERE u.vigente = 'SI' and u.tipoUnidadCodigo in (10,15) ORDER BY u.descripcionDependencia ASC")
    ,
  @NamedQuery(name = UnidadDependencia.FIND_ESCUELAS_VIGENTES, query = "SELECT u FROM UnidadDependencia u WHERE u.vigente = 'SI' and u.tipoUnidadCodigo in (10,15) and u.siglaPapa = 'DINAE' ORDER BY u.descripcionDependencia ASC")
    ,
